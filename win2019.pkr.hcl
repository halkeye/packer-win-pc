...
build {
  ...
  provisioner "powershell" {
    elevated_password = "packer"
    elevated_user     = "administrator"
    environment_vars  = ["wsus_server=${var.wsus_server}", "wsus_group=${var.wsus_group}"]
    remote_path       = "c:/Windows/temp/windows-update-winrm.ps1"
    script            = "scripts/windows-update-winrm.ps1"
  }

  # copy the script again because cleanup will always happen on elevated scripts https://www.packer.io/docs/provisioners/powershell#skip_clean
  provisioner "file" {
    destination = "c:/Windows/temp/windows-update-winrm.ps1"
    source      = "scripts/windows-update-winrm.ps1"
  }
  ...
  provisioner "powershell" {
    inline      = [
      "get-content c:/windows/temp/windows-update-winrm.log | format-wide | write-output"
    ]
